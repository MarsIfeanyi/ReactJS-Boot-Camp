 ...
 *
 * Hint: We are just building our own custom HTML and again just as HTML has attributes, it turns out that with React, our own custom components can also have attributes.
 *
 * Props = Properties...we can set properties of our own custom components
 *
 * Hint: We add the attributes to the custom HTML we created in App.js
 *
 ...
 *
 * The props have key-value pairs as well. The keys are the attributes name defined in the custom HTML inside the App.js file, and the values is the values set in the {}
 *
 * Hint: The key you access from your props object has to be the same as the name you used for attribute.
 *
 * Hint:Choose/use attribute names which are self explanatory and makes sense.
 *
