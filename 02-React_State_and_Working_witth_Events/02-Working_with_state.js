 ...
 *
 * Hint:All hooks are recognized with the 'use' that starts their name, ie all hooks starts with the 'use' keyword
 *
 ...
 *
 * Hint: useState always returns an array with exactly two elements, the 1st element is the current state value and the 2nd element is a function for updating the current state value
 *
 *
 * Hint: When you call the state updating function (eg. setTitle), it doesn't just assign a new value to some variable, but the component function in which you call the state updating function and in which you initialize your state with useState will be executed again and re-valuated
 *
 *
 * Hint: If you have data which might change and where changes to that data should be reflected on the user interface, then you need state, because regular variables will not do the trick, with state however you can set and change values and when they change React will re-evaluate the component in which the state was registered.
 *
